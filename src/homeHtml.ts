  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .dir-group-header:hover { background: #f3f4f6; }
      .dir-group-header .count { font-size: 11px; color: #9ca3af; }
      .dir-items { overflow: hidden; transition: max-height 0.2s ease; }
      .dir-items.closed { max-height: 0 !important; }
      .dir-item {
        display: block;
        width: 100%;
        text-align: left;
      ...
      .dir-item:hover { background: #f3f4f6; }
      .dir-item.active {
      ...
      .dir-toggle.collapsed { transform: rotate(-90deg); }
      .sidebar-mask { display: none; }
      .sidebar-toggle { display: none; }
      @media (max-width: 768px) {
        .layout { grid-template-columns: 1fr; }
        .sidebar { display: none; position: fixed; top: 0; left: 0; width: 280px; height: 100vh; z-index: 90; border-radius: 0; max-height: 100vh; }
        .sidebar.open { display: block; }
        .sidebar-mask { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.3); z-index: 89; }
  ...
  <body>
          ...
          <section class="card feed">
            <div class="section-head">
              <div style="display:flex;align-items:center;gap:8px;">
                <button class="sidebar-toggle" id="sidebarToggle" type="button" title="打开目录">☰</button>
                <div>
    ...
    <script>
      const state = {
        ...
        activeNoteId: null,
        expandedIds: new Set(),
        dirOpenGroups: new Set(['今天', '昨天', '本周']),
        searchTimer: null,
        statusTimer: null,
      ...
      function getDirGroupLabel(ts) {
        ...
        if (d.getTime() >= weekStart.getTime()) return '\u672c\u5468';
        const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
        if (d.getTime() >= monthStart.getTime()) return '\u672c\u6708';
        return '\u66f4\u65e9';
      }

      function toggleDirGroup(label) {
        if (state.dirOpenGroups.has(label)) {
          state.dirOpenGroups.delete(label);
        } else {
          state.dirOpenGroups.add(label);
        }
        renderDirectory();
      }

      function renderDirectory() {
        if (!els.dirTree) return;
        els.dirTree.innerHTML = '';
        if (!state.vaultUnlocked || !state.allNotes.length) {
          els.dirTree.innerHTML = '<div class="muted" style="font-size:13px;padding:8px;">暂无笔记</div>';
          return;
        }

        const groups = new Map();
        state.allNotes.forEach(function (note) {
          const key = getDirGroupLabel(note.updated_at);
          if (!groups.has(key)) groups.set(key, []);
          groups.get(key).push(note);
        });
        ...
        groupOrder.forEach(function (label) {
          ...
          const notes = groups.get(label);
          const isOpen = state.dirOpenGroups.has(label);
          ...
          header.onclick = function () { toggleDirGroup(label); };
          header.innerHTML = '<span><span class="dir-toggle' + (isOpen ? '' : ' collapsed') + '">\u25bc</span> ' + label + '</span><span class="count">' + notes.length + '</span>';

          const items = document.createElement('div');
          items.className = 'dir-items' + (isOpen ? '' : ' closed');

          notes.forEach(function (note) {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'dir-item' + (note.id === state.activeNoteId ? ' active' : '');
            btn.textContent = note.title || '\u65e0\u6807\u9898';
            btn.title = note.title || '\u65e0\u6807\u9898';
            btn.onclick = function () {
              state.activeNoteId = note.id;
              renderDirectory();
              renderList();
              // Scroll to the note card
              const cards = els.noteList.querySelectorAll('.note-card');
              for (let i = 0; i < cards.length; i++) {
                const c = cards[i];
                if (c.dataset.noteId === note.id) {
          ...
          els.dirTree.appendChild(groupDiv);
        });
      }
      ...
      if (els.sidebarToggle) {
        els.sidebarToggle.onclick = function () {
          ...
          els.sidebarMask.classList.toggle('open');
        };
      }
      if (els.sidebarMask) {
